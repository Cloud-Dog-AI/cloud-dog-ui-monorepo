...
export function EvalPage() {
  ...
  React.useEffect(() => {
    let c = false;
    void api.evaluation().then((d) => { if (!c) setData(d); }).catch((e) => { if (!c) setError(errMessage(e)); }).finally(() => { if (!c) setLoading(false); });
    return () => { c = true; };
  }, [api]);
  return (
    <div className="space-y-4">
      <PageHeader title="Eval" description="LLM-EVAL comparison harness results." />
      <StatusLine loading={loading} error={error} />
      <Card><CardHeader><h2 className="text-lg font-semibold">Eval results</h2></CardHeader>
