...
// MCP console (PS-72 v2) via the shared Ps72McpConsole. chart-mcp exposes a JSON-RPC
...
type RawTool = Readonly<{ name: string; description?: string; input_schema?: unknown; inputSchema?: unknown }>;

export function McpConsolePage() {
  const auth = useAuth();
  const { api } = useChartState();
  const origin = window.location.origin;
  const rpcUrl = `${origin}/mcp`;
  ...
  const onExecute = React.useCallback(
    async (toolName: string, args: unknown): Promise<Ps72ExecuteResult> => {
      const call = await api.mcpCall(toolName, args);
      const body = call.body;
      const rpcResult = body && typeof body === "object" ? (body as Record<string, unknown>).result : null;
      const structured = rpcResult && typeof rpcResult === "object" ? (rpcResult as Record<string, unknown>).structuredContent : null;
      const jobId =
