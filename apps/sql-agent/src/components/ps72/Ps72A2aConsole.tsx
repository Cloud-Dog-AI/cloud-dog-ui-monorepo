...
// @cloud-dog/app-sql-agent — PS-72 v2 A2A Console (W28A-774).
//
// Implements PS-72 v2 §1-§9 plus §8 A2A specifics (agent card + events stream)
// that the shared @cloud-dog/ui A2aConsole pattern does not yet carry.

import * as React from "react";
import { Button, Input, Textarea } from "@cloud-dog/ui";
    ...
    <div data-testid="a2a-console-page" className="flex flex-col gap-3">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <Ps72HealthBadge state={props.health} testId="a2a-console-status-badge" />
        </div>
        <a data-testid="a2a-console-docs-link" href={props.docsHref} className="text-sm font-medium text-sky-700 hover:underline">
          Docs
        </a>
      </div>
      ...
      <div className="flex rounded-md border bg-background">
          ...
          <div data-testid="a2a-console-agent-card" className="rounded-md border border-slate-200 bg-slate-50 p-3">
            ...
            {props.agentCard ? (
              <ul className="mt-2 space-y-1 text-xs text-slate-600">
                {props.skills.slice(0, 8).map((skill) => (
        ...
        <div className="min-w-0 flex-1 space-y-3 p-4">
          ...
          <label data-testid="a2a-console-request-label" className="block text-xs font-semibold uppercase tracking-wide text-slate-600" htmlFor="a2a-console-request-editor">
            Request
          </label>
          <Textarea
            id="a2a-console-request-editor"
            data-testid="a2a-console-request-editor"
            value={requestText}
          ...
          <Button data-testid="a2a-console-submit" onClick={submit} disabled={running || !action.trim()}>
            {running ? "Submitting…" : "Submit"}
          </Button>

          <Ps72ResultMeta testIdPrefix="a2a-console" result={result} meta={meta} denied={denied} jobsHref={props.jobsHref} />
        </div>
      </div>
    </div>
  );
