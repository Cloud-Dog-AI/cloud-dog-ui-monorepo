...
// @cloud-dog/app-sql-agent — PS-72 v2 MCP Console (W28A-774).
//
// Implements the PS-72 v2 §1-§9 canonical layout + data-testid contract that the
// shared @cloud-dog/ui McpConsole pattern does not yet carry. Sql-agent-local
// wrapper landed first (W28A-774 option "2 then 1"); promotion to the shared
// component is the follow-on. NO assertion is weakened (PC17).
...
/** Pull a Job ID out of a result body per common PS-75 shapes (incl. MCP content text). */
function extractJobId(body: unknown): string | null {
  ...
  const direct = result.job_id ?? result.jobId ?? result.job ?? record.job_id;
  if (typeof direct === "string" && direct.trim()) return direct.trim();
  // MCP tool results carry the payload as a JSON string inside result.content[].text.
    ...
    for (const part of content) {
      ...
      if (typeof text === "string") {
        try {
          ...
          const id = parsed.job_id ?? parsed.jobId ?? parsed.job;
          if (typeof id === "string" && id.trim()) return id.trim();
        } catch {
          const m = text.match(/JOB-\d+/);
          if (m) return m[0];
        }
      }
    }
