...
export function A2aConsolePage() {
  const { api, apiKey, a2aBaseUrl, authMode, selectedProfile } = useFileMcpState();
  ...
  React.useEffect(() => {
    ...
    void Promise.all([
      ...
      fetch(`${a2aBaseUrl}/.well-known/agent.json`, { credentials: "include" }).then((response) => response.ok ? response.json() : null).then((card) => setAgentCard(card as Record<string, unknown> | null)).catch(() => undefined),
      ...
      <Card>
        ...
        <CardContent className="space-y-2 text-sm">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <a className="text-sm font-medium text-sky-700 hover:underline" href="/api-docs#a2a">View A2A documentation</a>
          </div>
      ...
      <Card>
        ...
        <CardContent>
          {loading ? <div className="flex items-center gap-2 text-sm text-muted-foreground" role="status" aria-live="polite"><Spinner className="h-5 w-5" />Loading A2A console...</div> : (
            <Ps72A2aConsole endpointUrl={a2aBaseUrl} agentCard={agentCard} skills={skillNames(agentCard)} health={healthState(agentCard, healthPayload)} hasBoundKey={authMode === "cookie" || Boolean(apiKey.trim())} boundLabel={boundKeyLabel(authMode, apiKey)} docsHref="/api-docs#a2a" jobsHref="/jobs" onSend={sendTask} />
          )}
        </CardContent>
      </Card>
    </div>
