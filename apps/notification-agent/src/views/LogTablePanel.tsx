...
// @cloud-dog/app-notification-agent — PS-40 NIST AU-3 compliant log DataTable panel.
// Covers: UI-R11, UI-R25, W28A-644
...
import type { AuditLogEntry, NotificationAdminApi, LogSurfaceId, LogsResponse } from '../lib/api';
...
function formatChannel(row: AuditLogEntry): string {
...
export function LogTablePanel(props: LogTablePanelProps) {
  ...
  const refresh = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const next = await api.listStructuredLogs({
        surface,
        limit,
  ...
  // NOTIFWEB-047: No delete action on audit/log entries (cross-project rule)
  ...
  const columns = React.useMemo<DataColumn<AuditLogEntry>[]>(() => [
    ...
    },
    {
      ...
      cell: (row) => row.timestamp ? <RelativeTime timestamp={row.timestamp} /> : <span className="font-mono text-xs">N/A</span>,
    },
    {
      id: 'traceId',
      header: 'Trace ID',
  ...
  const body = (
    <div className="space-y-4">
      <div className="flex flex-wrap items-end gap-3">
        ...
        <Button type="button" variant={followTail ? 'secondary' : 'default'} onClick={() => setFollowTail((current) => !current)}>
      ...
      <div className="space-y-1 text-sm text-muted-foreground">
        ...
        <p>Tail mode: {followTail ? 'Following with 30s refresh' : 'Paused'}{lastRefreshAt ? <>{' · last refresh '}<RelativeTime timestamp={lastRefreshAt} /></> : ''}</p>
      </div>
