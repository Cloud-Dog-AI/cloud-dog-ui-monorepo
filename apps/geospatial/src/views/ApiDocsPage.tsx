...
export function ApiDocsPage() {
  const { appVersion, api } = useGeoState();
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        if (!cancelled) setTools(normaliseTools(await api.mcpTools()));
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="mcp">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <DataTable columns={toolColumns} rows={tools} emptyMessage="No MCP tools." getRowId={(row) => row.name} tableId="geospatial-mcp-docs-tools" pageSize={25} columnPickerEnabled />
        ...
        <TabsContent value="a2a">
          <Card>
            ...
            <CardContent className="space-y-4">
              {loading ? <div className="flex items-center gap-2 text-sm text-muted-foreground"><Spinner className="h-5 w-5" /> Loading…</div> : null}
              <DataTable columns={skillColumns} rows={skills} emptyMessage="No A2A skills." getRowId={(row) => row.name} tableId="geospatial-mcp-docs-skills" pageSize={20} />
        ...
        <TabsContent value="readme">
          <Card>
            <CardHeader>
              <h2 className="text-lg font-semibold">geospatial-mcp-server</h2>
              <Badge variant="secondary">v{appVersion}</Badge>
            </CardHeader>
            <CardContent className="space-y-3 text-sm">
              <p>Geocoding, feature discovery, map rendering, imagery search, evidence bundles, report export, and asset governance across API, MCP, A2A and WebUI surfaces.</p>
              <ul className="list-disc space-y-1 pl-5 text-muted-foreground">
                <li><strong>REST API</strong> under <code className="text-xs">/api</code>: providers, profiles, sessions, geocode, features, imagery, map render, evidence, reports, assets, jobs.</li>
                ...
                <li><strong>Auth</strong>: cookie session via <code className="text-xs">/auth/login</code>; the web tier bridges the session to scoped identity headers.</li>
              </ul>
            </CardContent>
          </Card>
        </TabsContent>
