...
// PS-74 Docs Page — code-runner: authenticated MCP/A2A discovery (PC15).
...
export function ApiDocsPage() {
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        const json = await api.request<unknown>("/webmcp/tools", { headers: authFetchInit.headers as Record<string, string> });
      ...
      try {
        const card = await api.request<Record<string, unknown>>("/weba2a/.well-known/agent.json", { headers: authFetchInit.headers as Record<string, string> });
        if (cancelled) return;
        setAgentCard(card);
  ...
  }, [agentCardUrl, api, authFetchInit, baseOrigin]);

  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-2xl font-semibold">API Docs</h1>
        <p className="text-sm text-muted-foreground">OpenAPI, MCP tool catalogue, and A2A skills for code-runner (PS-74).</p>
      </header>
        ...
        <TabsContent value="a2a">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {skills.length > 0 ? (
                <DataTable
                  tableId="code-runner-api-docs-a2a-skills"
